...
const Preview = () => {
  ...
  const setArraySelectedIndex = (index: number) => {
  ...
  return (
    <div className={"w-80 flex flex-col gap-4"}>
      ...
      <ToggleButtonGroup>
        <ToggleButton startIcon={<FootPrintIcon />} selected={arraySelected[0]} onClick={() => setArraySelectedIndex(0)}>
          Walking
        </ToggleButton>
        <ToggleButton startIcon={<TrainIcon />} selected={arraySelected[1]} onClick={() => setArraySelectedIndex(1)}>
          Transit
        </ToggleButton>
        <ToggleButton startIcon={<DirectionsCarIcon />} selected={arraySelected[2]} onClick={() => setArraySelectedIndex(2)}>
          Driving
        </ToggleButton>
      </ToggleButtonGroup>
    </div>
