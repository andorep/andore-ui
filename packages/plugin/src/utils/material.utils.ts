...
export const isValidMaterialDesignThemeBuilderJSON = (theme: MaterialDesignThemeBuilderJSON) => {
    const {schemes} = theme;
    const {light, dark} = schemes;
    const missingKeys = requiredColors.filter((key) => {
        return !light[key] || !dark[key];
    });
    if (missingKeys.length > 0) {
        throw new Error(`Missing keys in theme: ${missingKeys.join(", ")}`);
    }
    return true;
}
